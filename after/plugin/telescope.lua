...
vim.keymap.set('n', '<leader>puf', function()
    ...
    builtin.find_files({
        prompt_title = "Search files",
        find_command = { 'fdfind', ".", userdir, '--hidden', '--exclude', '.git', '--exclude', '.cache', '--exclude', '.local', '--exclude', '.npm', '--exclude', '.cargo', '--max-depth', '5', '--type', 'f' }
...
vim.keymap.set("n", "<leader>prf", function()
    builtin.find_files({
        ...
        cwd = "/",
        hidden = true,
        find_command = { 'fdfind', '--full-path', '/', '--hidden', '--exclude', '.*', '--max-depth', '3', '--type', 'f' }
